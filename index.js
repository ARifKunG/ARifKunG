// --------- Firebase Config (แทนค่าด้วยของคุณเอง!) -----------
...
listenFirestore(ERROR_COL, arr => {
  errorData = arr;
  // อัปเดต UI หลังข้อมูลเปลี่ยน (ถ้ากำลังค้นหาอยู่ ให้ render ตามฟิลเตอร์ & คำค้น)
  errorSearchHandler();
});
...
function renderErrorListByField(field, val) {
  ...
  if(field === "brand") {
    list = errorData.filter(e => (e.brand||"").toLowerCase() === val);
  }
  if(field === "model") {
    list = errorData.filter(e => (e.model||"").toLowerCase() === val);
  }
  if(field === "errorCode") {
    list = errorData.filter(e => (e.errorCode||"").toLowerCase() === val);
  }
  if(!list.length) {
